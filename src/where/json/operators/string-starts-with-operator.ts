...
export class StringStartsWithOperator extends BaseOperator<string> {
  readonly key = 'string_starts_with' as const;
  ...
  preprocessValue(value: unknown): string {
    if (typeof value !== 'string') {
      throw new Error('string_starts_with requires a string value');
    }
    return value;
  }
  ...
  getErrorMessage(context: string): string {
    switch (context) {
      case 'validation failed':
        return 'string_starts_with requires a non-empty string value';
      default:
