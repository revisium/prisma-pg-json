...
import type { SubSchemaTableConfig } from '../../sub-schema/types';
import { unquotePath } from '../../sub-schema/path';
import type { ParsedPath, PathSegment } from '../../sub-schema/path';
import { describeSubSchemaCte } from '../../sub-schema/cte-description';
import {
  getEmptyCteSelect,
  buildPathAccess,
  buildJsonPathAccess,
 ...
 */
export function compileSubSchemaCte(
  tables: SubSchemaTableConfig[],
  cteName = 'sub_schema_items',
): PrismaSql {
  if (tables.length === 0) {
...
export function compileSubSchemaCteQueries(tables: SubSchemaTableConfig[]): PrismaSql {
  const queries: PrismaSql[] = [];

  for (const { table, parsed } of describeSubSchemaCte(tables)) {
